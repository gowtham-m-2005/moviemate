...
def watch_time_stats(db: Session = Depends(get_db)):
    ...
    for m in movies:
        ...
        if m.platform:
            by_platform[m.platform] = by_platform.get(m.platform, 0) + (
                m.watch_minutes or 0
            )
...
def get_recommendations(db: Session = Depends(get_db)):
    ...
    history = [
        {"title": m.title, "genre": m.genre, "rating": m.rating, "status": m.status}
        for m in watched
    ]
    ...
    try:
        ...
        for rec in recs:
            try:
                ...
                res = tmdb_session.get(
                    f"{BASE}/search/{kind}?api_key={TMDB_KEY}&query={rec['title']}",
                    timeout=5,
                    verify=False,
                )
                ...
                if results and results[0].get("poster_path"):
                    rec["poster_url"] = (
                        f"https://image.tmdb.org/t/p/w300{results[0]['poster_path']}"
                    )
